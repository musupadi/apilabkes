	...
	private function _get_api_list(){
		...
		if ($handle = opendir(self::CONTROLLER_DIR)) {
			while (false !== ($entry = readdir($handle))) {
				if ($entry != "." && $entry != ".." 
				&& strpos($entry, ".php") 
				&& !preg_match('/^(?:docs.php)$/i', $entry) 
				&& !preg_match('/^(?:welcome.php)$/i', $entry)
				&& !preg_match('/^(?:google.php)$/i', $entry)
				&& !preg_match('/^(?:example_api.php)$/i', $entry)) {
		...
		foreach(explode('function', $this->_get_controller_source($file_name)) as $api_str){
			...
			if (count($api_method)){
				...
				array_push($api_list, array(
					...
					'header' => (!in_array($api_method_name,$arrayExcludeHeader)) ? ['X-API-KEY'] : [], //$api_header[1], //'access_token',
					'call_type' => $call_type,
